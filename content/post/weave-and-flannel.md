---
title: "Weave/Flannel/Calico 的转发流程"
date: 2022-04-09T16:59:44+08:00
draft: false
---
这篇文章主要讨论 weave 和 flannel 各自怎么使用 vxlan 实现跨主机网络的情况。从网络设备、主机路由、IP管理、CNI实现、容器路由、vxlan发现几个角度来比较。最后再简单介绍一下 calico 的转发流程。
...
在对端的 weave bridge 能抓到 arp 广播和响应，非查询本机 pod 的 arp 也可以收到。
...
1. 如果目的地址是本机，直接发送 arp 广播，源IP写的是src pod IP，要查询的地址是 dst pod IP 。这个 arp 广播会被所有通过veth pair连接到 cni0 bridge 的网卡收到，也就是本机的 pod 都能收到，并相应 arp，告知 mac，继续往后的通信流程。
...
1. IPAM：weave是自己管理和分配，是一个大子网，不区分 node；flannel 是由 kube-controller-manager （分配 node 子网）和 host-local cni 分配（分配 pod ip）。
...
6. cni 插件实现：weave 自己实现了 pod 网卡准备和 IPAM； flannel 主要还是用了标准的 bridge 和 host-local cni 插件。
7. 路由信息的交换：weave 是通过 tcp 和 gossip 协议来交换信息；flannel 依赖了 k8s 的 API 和 node 对象（信息写在 metadata.annotaion 中）。

## 4. calico

### 有哪些网络设备
不同于 weave 和 flannel，calico 不会设置一个 bridge，不会把 veth 挂到 brigde 下。

每个 veth 的 mac 地址都是 `ee:ee:ee:ee:ee:ee`，没有 IP 地址，一般用 `cali*`命名。

### 主机路由和网段划分
1. 每个本机容器会有一条路由，指定路由到哪一个 veth
2. 对于其他的node，各自至少有一条掩码为 24 的路由
### CNI 阶段
calico 的 cni 实现比较复杂：

1. 会使用 calico 和 k8s 的客户端，获取和更新 workload 的各种信息
2. 创建容器网络 ([Dataplane.DoNetworking](https://github.com/projectcalico/calico/blob/36fef94f8601a4e20d830df44ba8310df62ba63c/cni-plugin/pkg/dataplane/dataplane.go#L34))
### 跨主机网络通信过程（如何区分目的 pod 在本机还是跨主机）
1. 先看容器内路由
```
...
```
14:36:40.000190 ARP, Request who-has 169.254.1.1 tell 10.144.189.72, length 28
14:36:40.000239 ARP, Reply 169.254.1.1 is-at ee:ee:ee:ee:ee:ee, length 28
```
