...
这其实与[求值策略(Evaluation strategy)](https://en.wikipedia.org/wiki/Evaluation_strategy)相关的概念。在传递参数的时候，编译器是怎么进行求值的，是否会发生内存的复制，不同的语言有自己的规定。不了解所使用的编程语言的规定，就很容易出错，也很容易写出低效率的代码。
...
2. 按引用传递（Pass by reference）：传递给函数的是它的实际参数的`隐式引用`而不是实参的拷贝。修改形参会改变实参。（发生了引用的复制）
...
这其实也是因为`复制`、`赋值`、`引用`等术语本身定义不够清晰引发的误会。知乎上[有个回答](https://www.zhihu.com/question/20628016/answer/86977962)提到`复制`的种类：

* 复制 value （按值传递）
* 复制地址 （按共享对象传递）
* 别名 （按引用传递）

而`赋值`也有两个涵义，而传递参数也是一种赋值（尽管在大多数语言里，这两个动作表现出来都`a=b;`的形式）：
...
按共享对象传递像是了结合了前两种方式，试图达到灵活度和性能之间的平衡。编译器和 GC 可以做出更好的管理。按共享对象传递概念的出现也是对前两种方式的补充。

不管如何，都要了解自己使用的工具的特性，写出保证正确性和效率的代码。
